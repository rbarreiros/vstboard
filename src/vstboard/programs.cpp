...
void Programs::rowsRemoved( const QModelIndex & parent, int start, int end )
{
        ...
        if(currentGrp.isValid()) {
            ...
            for(int i=0; i<model->rowCount(); i++) {
                if (model->hasIndex(i,0)) {
                    ...
                    if(index.data(UserRoles::value).toInt()==1) {
    ...
    for(int i=0; i<parentItem->rowCount(); i++) {
        if (model->hasIndex(i,0,parentIndex)) {
            ...
            if(indexPrg.data(UserRoles::value).toInt()==1) {
                ChangeProg(indexPrg);
                return;
            }
        }
...
void Programs::BuildModel()
{
    ...
    for(unsigned int grp=0; grp<3; grp++) {
        ...
        nextGroupId++;
        grpItem->setData(0,UserRoles::value);
...
//            prgItem->setData(nextProgId,Qt::ToolTipRole);
            nextProgId++;
            prgItem->setData(0,UserRoles::value);
    ...
    model->item(0)->setData(1,UserRoles::value);
    model->item(0)->child(0)->child(0)->setData(1,UserRoles::value);
...
void Programs::ChangeProg(const QModelIndex &newPrg)
{
    ...
    if(newgrp!=currentGrp) {
        ...
        if(currentGrp.isValid()) {
            ...
            model->itemFromIndex( currentGrp )->setData(0,UserRoles::value);
        } else {
        ...
        if(newGrpItem) {
            ...
            newGrpItem->setData(1,UserRoles::value);
...
//                newGrpItem->child(i,0)->setBackground(Qt::transparent);
    ...
    if(currentPrg.isValid()) {
        ...
        model->itemFromIndex(currentPrg)->setData(0,UserRoles::value);
    } else {
        debug("Programs::ChangeProg old prog not found")
    }

    model->itemFromIndex(newPrg)->setBackground(Qt::green);
    model->itemFromIndex(newPrg)->setData(1,UserRoles::value);
